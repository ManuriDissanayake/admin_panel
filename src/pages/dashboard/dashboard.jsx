...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // Fetch recent orders
        const ordersResponse = await fetch('http://localhost:8080/admin/orders/recent?limit=5')
  ...
  const handleMainCategoryChange = async (categoryId) => {
    try {
      ...
      let data;
      if (categoryId) {
        // Fetch filtered category distribution from API
        ...
        // Get all categories distribution from API
